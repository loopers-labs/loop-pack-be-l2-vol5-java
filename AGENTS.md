...
- Java 21 문법과 현재 프로젝트의 Spring 사용 방식을 따른다.
- Domain 계층에는 Lombok을 사용하지 않고, 필요한 접근자와 행위를 직접 선언한다.
- Infrastructure의 JPA Entity에는 반복되는 접근자 코드를 줄이기 위해 `@Getter`를 사용할 수 있다. 다만 `@Data`, `@Setter`는 사용하지 않고, 상태 변경은 의도가 드러나는 메서드로 표현한다.
- JPA Entity의 컬렉션은 생성된 `@Getter`로 내부 컬렉션을 그대로 노출하지 않는다. 필요한 경우 명시적인 접근자를 두고 컬렉션 변경 범위를 제어한다.
- 의존성 주입은 생성자 주입을 우선한다.
- 도메인 Entity는 생성 의도가 드러나는 정적 팩토리 메서드(`create`)로 만든다. JPA 영속화를 위한 `protected` 기본 생성자는 예외로 둔다.
- 응답 DTO 변환은 `from` 정적 팩토리 메서드를 사용한다. 단순 VO와 요청 DTO에는 정적 팩토리를 강제하지 않는다.
