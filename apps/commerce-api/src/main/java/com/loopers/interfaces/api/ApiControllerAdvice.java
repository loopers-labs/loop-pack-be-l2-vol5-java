...
public class ApiControllerAdvice {
    ...
    public ResponseEntity<ApiResponse<?>> handle(DomainException e) {
        log.warn("DomainException : failure={}, code={}, message={}", e.failure(), e.code(), e.getMessage());
        return ResponseEntity.status(statusOf(e)).body(ApiResponse.fail(e.code(), e.getMessage()));
    }

    private HttpStatus statusOf(DomainException e) {
        return switch (e.failure()) {
            case RULE_VIOLATION -> HttpStatus.CONFLICT;
            case UNIDENTIFIED -> HttpStatus.NOT_FOUND;
    ...
    public ResponseEntity<ApiResponse<?>> handleBadRequest(HttpMessageNotReadableException e) {
        ...
        if (e.getCause() instanceof ValueInstantiationException valueInstantiation) {
            String fieldPath = valueInstantiation.getPath().stream()
                .map(ref -> ref.getFieldName() != null ? ref.getFieldName() : "?")
                .collect(Collectors.joining("."));
            Throwable reason = valueInstantiation.getCause();
            String detail = reason != null && reason.getMessage() != null
                ? reason.getMessage()
                : valueInstantiation.getOriginalMessage();
            errorMessage = fieldPath.isEmpty()
                ? detail
                : String.format("필드 '%s' 의 값이 거절되었습니다: %s", fieldPath, detail);
        } else if (rootCause instanceof InvalidFormatException invalidFormat) {
