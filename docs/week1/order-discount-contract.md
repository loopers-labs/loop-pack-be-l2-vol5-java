...
|---|---|---|---|---|---|---|
| 1 | 존재하는 숫자 ID | 200 | `SUCCESS` | 없음 | 있음 | 없음 |
| 2 | `abc` | 400 | `FAIL` | `Bad Request` | 없음 | 요청 파라미터 'exampleId' (타입: Long)의 값 'abc'이(가) 잘못되었습니다. |
| 3 | 존재하지 않는 숫자 ID | 404 | `FAIL` | `Not Found` | 없음 | [id = -1] 예시를 찾을 수 없습니다. |
| 4 | 미매핑 URL | 404 | `FAIL` | `Not Found` | 없음 | 존재하지 않는 요청입니다. |
...
{"meta":{"result":"FAIL","errorCode":"Not Found","message":"[id = -1] 예시를 찾을 수 없습니다."}}
{"meta":{"result":"FAIL","errorCode":"Not Found","message":"존재하지 않는 요청입니다."}}
```

위에서부터 정상 ID, `abc`, 존재하지 않는 숫자 ID, 미매핑 URL의 응답이다. `null`인 필드는 직렬화에서 제외되므로, 성공 응답에는 `errorCode`·`message`가, 실패 응답에는 `data`가 아예 나타나지 않는다.
...
|---|---|---|---|
| 1 | 할인액이 원금을 초과할 때 원금까지만 적용(캡)하는가, 적용을 거절하는가? | 사용자 응답(성공/실패), 저장 데이터(쿠폰 소진 여부) | 5장 검증 흐름 5번의 분기, INV-003 기대 결과 |
...
| # | 항목 | 처리 | 이유 |
|---|---|---|---|
| 1 | 쿠폰 사용 횟수 제한 및 동시 사용 시 한도 초과 제어 | 거절 | 과제 범위 밖으로 선언. 횟수 제한 기능이 없으면 동시성 문제도 발생하지 않음 |
| 2 | 최소 주문 금액, 특정 카테고리 한정 조건 | 거절 | 일반적인 구매 흐름의 필수 요소가 아님. 유효기한만 쿠폰 개념에 내재된 속성으로 판단해 남김 |
| 3 | 쿠폰 발급 주체·발급 선행조건·복수 발급 | 거절 | 이번 설계는 이미 발급된 쿠폰의 적용만 다룸 |
| 4 | 쿠폰 폐기 시 확정된 할인도 무효화되는가 | 보류 | INV-004는 어떤 이유로도 재계산하지 않는 것으로 정의했다. 다만 어뷰징 등으로 발급된 쿠폰을 회수해야 하는 정책이 생기면 INV-004 자체를 수정해야 하므로 질문을 닫지 않는다 |
| 5 | AI 도구가 제안한 항목 | 해당 없음 | 채택한 제안 없음 |
...
- **반례 2**: 소유권 검증을 다른 검증보다 뒤에 두면, 앞선 검증의 응답 차이만으로 남의 주문 상태가 노출된다. 규칙이 있어도 실행 순서가 틀리면 무력화된다. 5장 검증 흐름에서 이 검증의 위치를 고정한 근거다.
...
`meta.result`는 `SUCCESS`/`FAIL` 두 값뿐이며, `null`인 필드는 직렬화에서 제외된다. 따라서 실패 응답에는 `data`가 나타나지 않는다.
...
|---|---|---|---|---|
| 1 | 소유자 불일치 | `NOT_FOUND` | 404 | 대상 재확인 (주문 존재 여부는 노출되지 않음) |
| 2 | 주문 없음 | `NOT_FOUND` | 404 | 대상 재확인 |
| 3 | 이미 확정된 주문 | `CONFLICT` | 409 | 이 주문으로는 불가, 다른 흐름으로 |
| 4 | 이미 다른 쿠폰 적용됨 | `CONFLICT` | 409 | 기존 쿠폰 해제 후 재시도 (제품 질문 4에 의존) |
| 5 | 쿠폰 만료·미보유 | `BAD_REQUEST` | 400 | 다른 쿠폰 선택 후 재요청 (입력 수정) |
...
**남는 한계**: "이미 확정된 주문"과 "이미 다른 쿠폰 적용됨"이 같은 409이고, `errorCode`가 HTTP reason phrase이므로 클라이언트가 둘을 구분할 수단은 `message` 문자열뿐이다. 두 경우의 다음 행동이 다르므로(전자는 불가, 후자는 해제 후 가능) 구분이 필요하다면 `ErrorType` 확장을 제품 담당자와 논의해야 한다. 이 한계는 1장에서 관찰한 기존 API의 동작(존재하지 않는 리소스와 미매핑 URL이 같은 `Not Found`)과 동일한 구조다.

### 6-2. 내부 경계 — 클라이언트가 몰라도 되는 것

| # | 결정 | 선택 | 버린 대안 | 버린 이유 |
|---|---|---|---|---|
| 1 | 오류 구분 | 도메인 예외와 HTTP 변환 분리 | 도메인에서부터 하나로 합치기 | 로그에서 권한 위반과 실제 부재를 구분 못 함 |
| 2 | 값 보존 | 확정 시점 snapshot 기록 | 조회 시마다 재계산 | 확정 금액이 정책 변경에 따라 바뀌어 INV-004 위반 |
| 3 | 책임 경계 | `Order.applyDiscount()`가 캡슐화 | Service가 순서 직접 조립 | 기능마다 검증 복사 필요, 한 곳만 빠뜨려도 보안 구멍 |
...
**책임 경계 보충** — INV-001은 실행 순서가 틀리면 무력화되므로(3장 반례 2), 검증 순서 자체를 도메인 안에 가두는 편이 안전하다. `Order.applyDiscount(coupon, requesterId, requestedAt)`의 호출자는 검증 항목도 순서도 알지 못한다.
