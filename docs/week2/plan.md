...
| v0.6 | 2026-09-17 | 포인트 결제 TDD 진행 (12-2), 변이 점검으로 W-5b·W-10 추가 |
| v0.7 | 2026-09-17 | 주문 확정·만료 배치 구현 반영, 설계 문서–구현 대조와 구현 리뷰 반영 (12-3, 13-1 #19~21) |
...
- **이 구조의 이름** — **layer-first 패키지 + 느슨한 계층형(relaxed layered)**이다. 패키지를 기능이 아니라 계층으로 먼저 나누고, 의존은 interfaces → application → domain 방향으로만 흐른다. interfaces는 유스케이스를 application에 맡기지만 **domain의 값 타입(`OrderStatus`·`PaymentMethod`·`ProductSort` 같은 enum, `OrderLines.Line`)은 직접 읽을 수 있다** — 위 표에서 interfaces가 피할 대상은 infrastructure뿐이다. 예외는 infrastructure 하나로, DIP 때문에 방향이 뒤집혀 domain에 의존한다. 🔧
- **느슨하게 둔 비용** — API 응답이 domain enum 이름에 묶인다. 도메인에서 `DRAFT`를 바꾸면 JSON도 바뀌므로 HTTP 테스트가 `"status": "CONFIRMED"` 같은 문자열을 확인한다. 엄격하게 막으려면 application에 응답용 타입을 두고 ArchUnit에 interfaces → domain 금지를 더한다. (v0.4의 "strict layered" 표기는 표·ArchUnit과 달라 v0.7에서 고쳤다.)
...
| 주문 생성 🔑 | `POST /api/v1/orders` | `{items[{productId, quantity}]}` | `id, status=DRAFT, items[{productId, productName, unitPrice, quantity, amount}], totalAmount, createdAt` | 401, 400, 404 |
| 주문 확정 🔑 | `POST /api/v1/orders/{orderId}/confirm` | — | 내 주문 상세와 같음 — `status=CONFIRMED`, `paidAmount`·`paymentMethod`·`confirmedAt` 채워짐 | 401, 404(남의·없는·남의 확정된 주문), 409(ORD-02~04) |
| 내 주문 목록 🔑 | `GET /api/v1/orders` | `page`, `size` | `content[{id, status, totalAmount, paidAmount?, createdAt}]` 최신순 | 401 |
| 내 주문 상세 🔑 | `GET /api/v1/orders/{orderId}` | — | `id, status, items[...], totalAmount, paidAmount?, paymentMethod?, confirmedAt?, createdAt` | 401, 404 |
...
- **고객·관리자 응답 필드의 차이** — 고객은 구매 판단에 필요한 `brand{name}`·`likeCount`를 보고, 상품·브랜드의 재고 수량·생성/수정 시각은 보지 않는다(자기 주문의 `createdAt`은 본다). 관리자는 운영에 필요한 `stock`·`brandId`·시각·구매자 `userId`를 본다.
- **관리자 접근 거절** — 일반 사용자·식별 없는 요청은 과제 제공 Security 설정에서 `403`으로 거절된다. 필터에서 끊기므로 이 응답은 `ApiResponse` 형식이 **아니다** (Spring 기본 오류 응답). 로컬 모의 관리자 경계라는 전제에서 수용한다. 이 설정에는 로그인 수단(httpBasic 등)이 없어 **앱을 실제로 띄우면 관리자 API는 항상 `403`**이다. 관리자 기능은 테스트에서 `ROLE_ADMIN` 사용자를 넣어 확인한다.
...
- **호출자가 모르는 것** — 컨트롤러는 조회·검증·차감 순서를 모른다. Product는 포인트를, PointGroup은 주문과 다른 그룹을 모른다. 어떤 주문에 쓰였는지는 PointGroup이 아니라 사용 이력의 주문 번호가 안다.
- **구현** ✅ — `OrderFacade.confirm`. "DRAFT야?"는 `OrderModel.checkConfirmable()`, "낼 수 있어?"는 `PointWalletPolicy.checkPayable()`이고, 변경 메서드(`confirm()`·`pay()`)도 같은 확인을 다시 부른다. 재고·그룹의 남은 금액은 같은 트랜잭션에서 조회한 엔티티의 변경 감지로 반영된다.
- **순서를 지키는 곳** — 확인을 변경보다 앞에 모은 순서는 트랜잭션 롤백 때문에 DB 상태로는 드러나지 않는다(확인 호출을 지워도 변경 메서드의 확인 + 롤백이 되돌린다). 호출 순서를 검사하는 테스트는 두지 않고 이 흐름도와 코드 리뷰로 지킨다. 차이가 드러나는 때는 되돌릴 수 없는 부수효과(외부 결제·이벤트)가 생길 때다.
- **전제** — 품목마다 확인하고 품목마다 차감하는 구조는 P-01(한 주문에 상품당 품목 하나)에 기댄다. 같은 상품을 여러 품목으로 허용하면 확정 판단이 상품별 합산 수량으로 확인해야 한다.
...
- **구현** ✅ — `PointExpirationFacade`(묶음 1,000개)와 `interfaces.scheduler.PointExpirationScheduler`(매일 04:00 서울, `point.expiration.cron`으로 변경 🔧). 스케줄러는 컨트롤러처럼 진입점이라 interfaces에 둔다.
- **묶음 하나 = 트랜잭션 하나** — Facade 안에서 `TransactionTemplate`으로 묶음마다 트랜잭션을 연다(ADR-11의 예외).
- **끝없이 돌지 않게** — 묶음은 실제로 금액을 만료한 그룹 수로 세고, 0원 만료 이력은 남기지 않는다. 변이 점검에서 "남은 금액 > 0" 조회 조건을 지우자 이미 처리한 그룹이 계속 다시 조회돼 무한 반복하는 것을 발견하고 넣었다.
...
| ↳ 동시 주문 확정 | 재고 초과 판매, 같은 포인트 그룹의 남은 금액 갱신 유실. **사용 이력은 두 줄 모두 남아 "그룹의 이력 합 = 남은 금액"(4-1)이 깨진다** — 남은 10,000에서 7,000 확정 두 건이 동시에 커밋되면 남은 3,000, 이력 −7,000 × 2, 두 요청 모두 200 (도메인 리뷰) | 재고는 조건부 UPDATE 또는 비관적 락, 포인트 그룹은 비관적 락 또는 `@Version` |
| ↳ 브랜드 삭제 vs 상품 등록 | 삭제된 브랜드에 살아 있는 상품이 생길 수 있다 | 브랜드 행 잠금 후 확인 |
| ↳ 같은 행을 바꾸는 요청끼리 (구현 리뷰에서 발견) | 엔티티 UPDATE가 전체 컬럼을 쓰므로, 상품 수정과 재고 변경(또는 주문 확정의 재고 차감)이 동시에 오면 나중에 커밋한 쪽이 먼저 바꾼 값을 덮어쓴다 (예: 재고 0 설정이 5로 되돌아감, 확정 중 관리자가 바꾼 가격이 옛 가격으로 되돌아감) | 동시 요청 요구가 생길 때 버전 비교(`@Version`) 또는 바뀐 컬럼만 쓰는 UPDATE와 함께 검토 |
| ↳ 주문 확정 vs 만료 배치 | 만료 직전에 시작한 확정과 만료 뒤에 도는 배치가 같은 그룹을 바꾸면 한쪽 변경이 덮어써진다 (만료로 비운 0이 되살아나거나 그 반대) | 동시 주문 확정과 같이 포인트 그룹 비관적 락 또는 `@Version` |
...
- **구현 순서** — 만료 배치는 체크리스트 항목이 아니므로 **체크리스트 기능을 먼저 끝낸 뒤** 구현한다. PNT-07(만료 그룹 제외)은 결제 규칙이므로 포인트 기능과 함께 구현한다. → 주문 확정까지 마친 뒤 v0.7에서 구현했다 (8-1).
...
- **예외** — 만료 배치는 묶음마다 커밋해야 하므로 Facade 메서드가 트랜잭션을 열지 않고, 안에서 `TransactionTemplate`으로 묶음마다 연다 (ADR-10, 8-1).
...
| 저장과 시각 ✅ 구현 | 저장소의 `save`는 `saveAndFlush`로 구현하고, 수정 후에도 `save` 반환값으로 응답을 만든다 | `@PreUpdate`(수정 시각)가 커밋 시점 flush에서 실행돼, 그 전에 만든 응답의 `updatedAt`이 이전 값이었다 (구현 리뷰에서 발견, E2E로 확인) 사용 이력·만료 이력은 추가만 하고 응답에 쓰지 않아 `save`를 그대로 쓴다. |
| 확정의 변경 반영 ✅ 구현 | 재고·그룹의 남은 금액은 `save` 호출 없이 변경 감지로 반영한다 | 같은 트랜잭션에서 조회한 관리 상태 엔티티다. 마지막 주문 `saveAndFlush`가 영속성 컨텍스트 전체를 flush한다. 명시적 `save`는 품목·그룹 수만큼 flush만 늘린다 (도메인 리뷰에서 확인) |
...
| application | 객체 협력, 요청자 구분, 삭제 조건, 롤백 | 실제 Spring 빈 + 테스트 DB | ORD-02~05 확정(거절 시 주문·재고·남은 금액·이력 네 가지 유지), ORD-05 롤백(확정된 주문 저장에서 실패시켜 이미 바뀐 값의 복원 확인), 만료 Facade 직접 호출(스케줄러 시각을 기다리지 않음, 묶음 커밋·중간 실패 후 재실행), 좋아요 동시성(ADR-13) |
...
| HTTP | 실제 응답과 저장 동작, 입력·접근 거절 시 기존 값 유지 | `@SpringBootTest` + MockMvc / TestRestTemplate | USR-01 401, PNT-02 타입 오류 400, 관리자·일반·미식별 요청, 고객 POST가 Security에 막히지 않음, BRD-02 상품 있는 브랜드 삭제 409, LIK-05 내 좋아요 목록, ORD-02·04 확정 거절 후 상태 유지 |
...
- **대표 TDD 사례: W-5(잔액 부족 거절) + W-6(만료 이른 순 차감)** ✅ — Red(`test:`) → Green(`feat:`) → Refactor(`refactor:`)를 **단계마다 커밋**했다. 커밋에는 실패 출력이 남지 않으므로 각 바퀴의 Red 출력과 변이 점검 결과를 [12-2](#12-2-포인트-결제-tdd-진행-기록)에 남긴다.
- **W-5b·W-10·W-8 정상 짝은 변이 점검에서 추가했다** — 지금 구현이 이미 맞아 처음부터 초록이므로, Red는 **틀린 구현(변이)에 대고** 확인했다.
- **만료 시각이 같은 그룹의 순서(P-14 "먼저 충전한 그룹부터")** 🔧 — 도메인은 정렬 시 입력 순서를 유지하고, 저장소가 `expires_at, id` 순으로 넘긴다(`findRemainingByUserId`). 도메인 단위 테스트는 두지 않았다. 유효기간 설정이 바뀌지 않는 한 같은 만료 시각이면 충전 시각도 같아 실제로 순서가 갈리지 않는다. 설정이 바뀌면 저장소 순서(id)에 기대게 된다.
- **모든 경계값을 HTTP에서 반복하지 않는다** — 값 경계는 domain 단위 테스트에서, HTTP는 대표 오류 하나와 "거절 시 저장값 유지"만 확인한다.

### 12-2. 포인트 결제 TDD 진행 기록

> 2026-09-17 세션. 대상: `PointWalletPolicy.pay()`. 시작 구현은 "넣은 순서대로, 그룹마다 가능한 만큼 차감"(v0.3 ⑧에서 일부러 남긴 틀린 최소 구현).

| 바퀴 | 테스트 | Red — 실제 실패 출력 | Green — 바꾼 것 |
|---|---|---|---|
| 1 | W-6 | `expected: Money[amount=0] but was: Money[amount=3000]` (B의 남은 금액. A부터 10,000을 쓰고 B에서 2,000을 씀) | 루프 전에 만료 시각 순으로 정렬 |
| 2 | W-5 | `Expecting code to raise a throwable.` (3,000만 쓰고 조용히 반환) | 루프 전에 잔액 < 결제액이면 409 거절 |
| 3 | W-7b | `CoreException: 만료된 포인트는 사용할 수 없습니다. at PointGroup.use ← PointWalletPolicy.pay` (정렬하면 만료된 A가 맨 앞에 옴) | 만료된 그룹을 정렬 전에 제외 |
| 4 | W-7 | 처음부터 초록 — 2바퀴의 잔액 확인이 루프 전에 거절 | 없음 |
| Refactor | — | — | 변수 이름 `byExpiry` → `usableByExpiry`, 규칙 주석 정리. 구조 변경 없음 |

**세션 중 테스트 자체에서 잡은 오류** (Red를 봤다고 테스트가 옳은 것은 아니다)

- `containsExactly(tuple(5000), tuple(7000))` — 꺼낸 값은 `(PointGroup, Money)` 쌍이라 **맞는 구현도 실패**하는 assertion이었다. 첫 assertion에서 멈춰 실행되지 않고 가려져 있었다. `tuple(groupB, Money.of(5_000))`로 고쳤다.
- 테스트가 `@Nested class Pay` 밖에 있었다 → 안으로 옮겼다.

**변이 점검** — 제품 코드를 일부러 망가뜨려 실행했다.

| 변이 | 결과 | 잡은 테스트 |
|---|---|---|
| 만료 그룹 필터 제거 | 잡힘 | **W-7b만** (W-7은 통과 — W-7b를 추가한 이유) |
| 만료 시각 정렬 제거 / 늦은 순으로 뒤집기 | 잡힘 | W-6 |
| 잔액 확인 제거 | 잡힘 | W-5, W-7 |
| 잔액과 같은 금액도 거절 | **처음엔 살아남음** → W-5b 추가 후 잡힘 | W-5b |
| 0원 사용 건너뛰기 제거 | **처음엔 살아남음** → W-10 추가 후 잡힘 | W-10 |
| 충전 가능 경계를 1원 당김 | **정상 짝이 코드에 없었음** → W-8 정상 짝 추가 후 잡힘 | W-8(1원 충전 가능) |

### 12-3. 주문 확정·만료 배치 변이 점검

> 확정·배치는 대표 TDD 대상이 아니라 구현과 함께 테스트를 썼다. 그래서 테스트가 무엇을 막는지 제품 코드를 일부러 망가뜨려 확인했다.

| 대상 | 변이 | 결과 | 잡은 테스트 |
|---|---|---|---|
| 확정 | 롤백 끔 (`noRollbackFor`) | 잡힘 | 롤백 (`expected: DRAFT but was: CONFIRMED`) |
| 확정 | 롤백 끔 + 주문 저장을 변경 앞으로 | 잡힘 (스텁을 확정된 주문 저장으로 한정한 뒤) | 롤백 |
| 확정 | 상품 재검증(`orderConfirmPolicy.check`) 호출 제거 | 잡힘 | 가격 변경 409 (test 리뷰) |
| 확정 | 사전 확인(`checkConfirmable`·`checkPayable`) 호출 제거 | **살아남음 — 동치 변이** | 변경 메서드의 확인 + 롤백이 같은 결과를 낸다. 8장 "순서를 지키는 곳" |
| 만료 | 경계를 `<`로 (정각 제외) | 잡힘 | P-20 정각 경계 |
| 만료 | "남은 금액 > 0" 조회 조건 제거 | 잡힘 (처음엔 **무한 반복**) | 묶음 처리 2건 |
| 만료 | 묶음 반복 없이 한 번만 | 잡힘 | 묶음 처리 2건 |
| 만료 | 전체를 트랜잭션 하나로 | 잡힘 | 두 번째 묶음 실패 후 재실행 |
...
| 8 | **AI 설계 리뷰** — 과제 프롬프트 "책임이 겹치는 부분과 변경에 취약한 의존을 찾아줘. 대안 두 가지의 비용을 비교하고, 결정되지 않은 정책은 먼저 질문해줘"로 v0.1 검토 | 아래 9~14 | 지적마다 코드·빌드 파일로 사실을 확인한 뒤 학습자가 판정했다 |
...
| 16 | **구현 리뷰 — 포인트 도메인** (domain·test 리뷰어): 충전 -1 입력이 해석하는 객체 없이 500이 될 경로, W-8에 짝이 되는 정상 사례 없음, W-6·W-5 Red가 표의 잘못된 구현으로 실패하지 않을 구현, `@DisplayName`에 PNT ID 없음 | **채택** (W-8 정상 사례는 처음엔 충전 E2E로 대신했고, v0.6에서 도메인 테스트로 추가) | 코드로 확인. 결제 1차 구현을 바꾸고, 충전 유스케이스가 음수 입력을 400으로 해석. 그룹 단위 거절의 오류 종류·W-7 기대값은 v0.5 🤔-6에서 결정 |
...
| 19 | **설계 문서–구현 대조** (읽기 전용 에이전트): 5-2 "strict layered" 표기와 실제 interfaces → domain 의존, 포인트 지갑의 확인·결제 미분리, 12장 application 예시(BRD-02·LIK-05)의 테스트 위치, 13-1 #16 옛 상태, 7-1 `createdAt` 누락, 관리자 API가 실행 중 항상 403, PRD-03 테스트 이름, LIK 주석 번호 | **채택** (5-2는 표기 수정 🔧 — 학습자 확인 대기) / 일부 **기각** | 체크리스트를 떨어뜨릴 불일치는 없었다. 지적마다 코드로 확인해 반영. "만료 시각이 같으면 id 순" 저장소 테스트는 기각 — InnoDB가 인덱스로 읽을 때 동률을 PK 순으로 돌려줘, 쿼리의 `id asc`를 지워도 테스트가 통과한다 |
| 20 | **구현 리뷰 — 주문 확정** (domain·api-contract·test): 확정 가격 비교가 snapshot이 아닌 가격 필드를 봄, 상품 맵 null 보장이 암묵적, 롤백 테스트가 "저장이 마지막 단계"에 기댐, 그룹 조회 순서 미지정·남은 금액을 그룹과 묶지 않음, 404·401·409 `errorCode` 미확인, 남의 확정된 주문 404 미고정, 응답 id를 정규식으로 추출, 7-1 확정 응답 필드, ADR-09 동시 확정의 이력 불일치·확정 vs 만료 배치 | **채택** / 일부 **기각·보류** | 세 리뷰어 모두 "커밋해도 됨". 코드로 확인 후 반영. 롤백 스텁을 확정된 주문 저장으로 한정한 뒤, 롤백을 끄거나 저장을 변경 앞으로 옮긴 구현에서 실패함을 확인. 확인 → 변경 호출 순서를 검사하는 테스트는 기각 — 구조 검사이고 DB 상태로는 차이가 없다(8장). 롤백 예외를 `CoreException`으로 바꾸는 제안은 보류 — DB 오류 같은 인프라 실패를 대표하도록 `IllegalStateException` 유지 |
| 21 | **변이 점검 — 만료 배치**: "남은 금액 > 0" 조회 조건을 지운 구현에서 테스트가 실패하지 않고 멈춤 | **채택** | 이미 처리한 그룹이 다시 조회돼 묶음이 계속 가득 찼다 — 실제 배치도 같은 위험. 금액을 만료한 그룹 수로 묶음을 세고 0원 이력을 남기지 않게 바꾼 뒤, 네 가지 변이(경계 `<`, 조건 제거, 반복 제거, 한 트랜잭션)가 모두 테스트에서 실패함을 확인 |
...
| Content-Type이 없는 요청(415)·지원하지 않는 method(405)는? | 스타터 `ApiControllerAdvice`의 `Throwable` 처리로 500이 된다 (구현 리뷰에서 발견, 이번 기능 범위 밖). `Accept: text/plain`처럼 JSON을 받지 않는 확정 요청은 커밋 뒤 응답 변환에서 406이 되어, 재시도하면 409를 받을 수 있다 (API 리뷰, 추정) | 핸들러를 추가하면 `ErrorType`에 종류가 늘어난다 |
