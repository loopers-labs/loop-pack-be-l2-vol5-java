...
- 고객: 로컬 실습용 `X-USER-ID`로 식별한다. 식별 헤더의 누락·형식 오류와 없는 사용자를 거절하고, 본인의 좋아요·포인트·주문만 다룬다.
...
오류 클래스와 오류 코드에는 HTTP 상태·응답 타입을 넣지 않는다. 업무 오류의 코드·메시지를 담고, 웹 계층의 예외 처리기가 이를 HTTP 상태와 ApiResponse로 변환한다. 기존 starter의 ErrorType에 있는 HttpStatus도 구현 단계에서 제거하고 웹 계층으로 매핑을 옮긴다. 이때 기존 Example의 외부 응답 계약은 유지한다. 환경세팅 커밋은 starter를 보존하며, 오류 분리는 다음 구현에서 적용한다.
...
| 실습 데이터로 잔액 0의 PointBalance를 준비한 뒤 조회 | 저장된 잔액 0원 반환 |
...
| 좋아요 | 독립적인 객체 규칙이 생기면 추가한다. 필드 보관만 하는 객체에 형식적인 테스트를 만들지 않는다. | 사용자–상품 관계 등록·중복 방지·취소, 본인 목록과 삭제 상품 처리 |
...
통합 테스트는 실제 application·repository·테스트 DB를 연결한다. 과제에서 요구한 고객·관리자 API의 정상 결과와 대표 오류는 해당 도메인의 통합 테스트에서 확인한다. 저장 결과는 flush/clear 후 재조회한다. 관리자·일반 사용자·미식별 요청의 구분도 대표 관리자 API에서 확인하며, 모든 관리자 API마다 같은 권한 조합을 반복하지 않는다. 충전 10,000원 → 여러 품목 7,000원 결제 → 잔액 3,000원 흐름은 주문 통합 테스트에서 확인한다.

TestFixture, TestFixtureConfiguration, 공통 테스트 부모 클래스와 범용 API 호출 래퍼는 도입하지 않는다. 데이터 준비·요청·검증을 테스트 본문에서 읽을 수 있게 작성하고, 필요한 의존성만 직접 주입한다. 기존 starter의 Testcontainers 설정과 DB 정리 도구는 사용한다.
...
| 잔액 준비 | 실습·테스트에 필요한 userId의 PointBalance를 미리 저장한다. 테스트마다 필요한 잔액을 준비하고 종료 시 DB를 정리한다. | 실습용 사용자와 잔액은 테스트 준비 데이터로 저장하며 자동 잔액 생성 흐름을 추가하지 않는다. 테스트 실행 순서와 관계없이 같은 결과를 얻도록 다른 테스트의 데이터를 사용하지 않는다. |
