...
| Product | Entity | 이름·가격의 유효성, 재고의 유효성과 차감·최종값 설정, **"지금 팔 수 있는가"**, 주문에 복사할 이름·판매 단가. 브랜드는 식별자만 안다 |
| Like | Entity | 사용자–상품 관계 한 건. 식별자만 안다 |
| PointGroup | Entity | **충전 한 건이 만든 포인트 묶음.** 충전액·남은 금액·만료 시각을 가지고 "지금 쓸 수 있나", "남은 금액 안에서 이만큼 써 줘", "만료 처리해 줘"에 답한다 |
| PointHistory | Entity (추가만) | 어느 그룹에서 얼마가 **충전·사용·만료**됐는지 한 줄. 만든 뒤 바뀌지 않는다 |
| Order | Entity | 품목 목록·합계·상태·결제 결과. **결제할 금액을 자기 합계로 답하고, 확정 시 결제액을 스스로 고정한다.** 주문자는 식별자만 안다 |
...
| 포인트 지갑 판단 | Domain Service | 자기 상태 없이, **한 사용자의 포인트 그룹들(= 지갑)**을 함께 보고 잔액 계산·충전 가능 여부·결제 가능 여부·사용 순서와 배분을 답한다 (ADR-04) |
...
| PNT-07 | **만료 시각이 지난 그룹**은 만료 처리 전이라도 잔액·결제에서 제외한다 | A가 만료 시각을 지난 뒤 잔액 조회 → B의 남은 금액만 | 만료된 A밖에 없을 때 결제 → `409` (잔액 부족) | PointGroup ("지금 쓸 수 있나", 기준 시각은 호출자가 전달) |
...
| Money 매핑 | 구현 전 짧은 확인 테스트 | Order에 Money가 둘(합계·결제액)이라 `@Embeddable`이면 컬럼 이름 재정의가 필요하고, `AttributeConverter`면 QueryDSL의 `price_asc` 정렬이 막힐 수 있다 |
...
- **대표 TDD 규칙: 포인트 그룹 사용 (PNT-04~07)** 🔧 — "만료 이른 순 배분", "그룹 남은 금액 이하", "만료 그룹 제외", "부족하면 아무것도 바뀌지 않음"은 서로 다른 잘못된 구현을 잡는다(순서 무시·초과 차감·만료 무시·부분 차감 후 실패). Red → Green → Refactor를 각각 커밋해 PR에서 변화를 보여 준다. **테스트 기대값 목록은 TDD 시작 전에 학습자가 확정한다.**
...
| 스타터 Jackson 설정에서 잘못된 입력이 어떻게 읽히는가? `{"amount":"10000"}`(문자열), `1.5`(실수), `{}`(누락), 품목 배열 대신 객체 하나 | PNT-02·ORD-01대로 400이어야 함 | AI 리뷰에 따르면 문자열은 숫자로, 실수는 정수로 변환되고, 누락은 필드 타입에 따라 500이 될 수 있다. **HTTP 테스트로 먼저 실패를 확인**한 뒤 입력 처리 방식을 정한다 |
| 한 번에 충전할 수 있는 금액·보유 잔액에 상한이 있는가? | 저장 범위(long)까지 | 상한을 400·409 규칙으로 추가하면 PNT-03은 사실상 도달하지 않는다 |
