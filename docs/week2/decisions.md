...
- 코드가 상태마다 하나라서, API 계약의 규칙별 코드를 담을 수 없다. 재고 부족과 잔액 부족이 같은 코드로 나간다.
...
- enum 하나가 API 계약의 오류 코드 표와 1:1로 맞는다.
...
- 요청으로는 생길 수 없으므로, [API 계약](./api-contract.md#오류-코드)의 "요청과 관계없는 내부 오류"에 해당한다.
...
- C는 [API 계약](./api-contract.md#응답-형식)이 오류 코드를 하나만 담으므로 응답 형식을 바꿔야 한다.
